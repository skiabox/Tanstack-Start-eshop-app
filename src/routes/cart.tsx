...
import { type CartItem, type MutateCartFnInput } from "#/types/cart-types";
...
// TanStack Start compiles this into an RPC-style function that can be imported
// from client components while keeping the handler implementation on the server.
export const mutateCartFn = createServerFn({ method: "POST" })
  // Server functions cross a network boundary, so validate the single `data`
  // payload before the server handler receives it.
  .inputValidator((data: MutateCartFnInput) => data)
  .handler(async ({ data }: { data: MutateCartFnInput }) => {
    // Keep server-only cart logic in a `.server` module and import it inside the
    // handler, which matches TanStack Start's recommended file organization.
    const { addToCart, updateCartItem, removeFromCart, clearCart } =
      await import("#/data/cart.server");

    // One typed server function handles all cart mutations; callers select the
    // operation by passing an action in the validated payload.
    switch (data.action) {
      case "add":
...
function CartPage() {
  ...
  const cart = Array.isArray(data) ? { items: data } : data;
